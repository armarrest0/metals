 ...
 * @return  true
 */
function check_rent_money($log_id, $money)
{
    if(is_numeric($log_id))
    {
        $sql = 'SELECT rent_money FROM ' . $GLOBALS['ecs']->table('rent_paylog') .
              " WHERE id = '$log_id'";
        $amount = $GLOBALS['db']->getOne($sql);
    }
    else
...
function rent_paid($log_id)
{
    ...
    {
        /* 取得要修改的支付记录信息 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('rent_paylog') .
                " WHERE id = '$log_id'";
        $pay_log = $GLOBALS['db']->getRow($sql);
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            /* 修改此次支付操作的状态为已付款 */
            $sql = 'UPDATE ' . $GLOBALS['ecs']->table('rent_paylog') ." SET is_paid = '1' WHERE id = '$log_id'";
            $GLOBALS['db']->query($sql);


            /* 修改订单状态为已付款 */
            $sql = 'UPDATE ' . $GLOBALS['ecs']->table('supplier_rent') .
                            " SET is_pay_ok = '1', " .
                            " pay_time = '" . gmtime() . "', " .
                            " pay_type = '$pay_log[type]', " .
                            " paysn = '".$pay_log['paysn']."'" .                                    
                   "WHERE rent_id = '$pay_log[rent_id]'";
            $GLOBALS['db']->query($sql);
