...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT COUNT(*) AS total_order_num, " .$total_fee.
           ...
           " WHERE 1 " . order_query_sql('finished')." AND supplier_id=".$_SESSION['supplier_id'];
    $order_general = $db->getRow($sql);
    $order_general['total_turnover'] = floatval($order_general['total_turnover']);

    /* 取得商品总点击数量 */
    $sql = 'SELECT SUM(click_count) FROM ' .$ecs->table('goods') .' WHERE is_delete = 0'." AND supplier_id=".$_SESSION['supplier_id'];
...
/*------------------------------------------------------ */
 ...
 function get_orderinfo($start_date, $end_date)
 {
    ...
    $sql = 'SELECT COUNT(*) AS unconfirmed_num FROM ' .$GLOBALS['ecs']->table('order_info').
           ...
           " AND add_time < '" . ($end_date + 86400) . "' AND supplier_id=".$_SESSION['supplier_id'];
    ...
    $sql = 'SELECT COUNT(*) AS succeed_num FROM ' .$GLOBALS['ecs']->table('order_info').
           ...
           " AND supplier_id = 0 AND extension_code <> 'virtual_good'".
           // 代码增加   By  www.68ecshop.com End
           " AND add_time >= '$start_date' AND add_time < '" . ($end_date + 86400) ."' AND supplier_id=".$_SESSION['supplier_id'];
