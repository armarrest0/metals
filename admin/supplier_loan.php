...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here', "信誉贷申请列表"); // 当前导航
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('supplier_loan_list.htm');
}
...
elseif ($_REQUEST['act']== 'history')
{
    ...
    /* 显示商品列表页面 */
    assign_query_info();
    $smarty->display('supplier_loan_histroy.htm');   
}
/*------------------------------------------------------ */
//-- 查看、编辑供货商
/*------------------------------------------------------ */
...
function suppliers_list()
{
	...
	{
                $total_rank = $GLOBALS['db']->getOne("select sum(comment_rank) from ".$GLOBALS['ecs']->table("comment")." where supplier_id=".$row['supplier_id']." group by supplier_id");
                $total_order = $GLOBALS['db']->getOne("select count(*) from ".$GLOBALS['ecs']->table("order_info")." where supplier_id=".$row['supplier_id']." and pay_status=2");
                
                
                $total_goods_money = $GLOBALS['db']->getOne("select sum(goods_amount) from ".$GLOBALS['ecs']->table("order_info")." where supplier_id=".$row['supplier_id']." and pay_status=2 group by supplier_id");
                $total_shipping_money = $GLOBALS['db']->getOne("select sum(shipping_fee) from ".$GLOBALS['ecs']->table("order_info")." where supplier_id=".$row['supplier_id']." and pay_status=2 group by supplier_id");
                $total_money = $total_goods_money + $total_shipping_money;
                
                
                $avarage_rank = intval($total_rank / $total_order);
        
                $row['total_order'] = $total_order;
                $row['avarage_rank'] = $avarage_rank;          
