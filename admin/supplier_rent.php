...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here', "租金管理"); // 当前导航
    ...
    foreach($result['result'] as $k=>$val){
        $rent = $db->getRow("select * from ".$ecs->table('supplier_rent')." where supplier_id=".$val['supplier_id']);
        $result['result'][$k]['rent_money'] = $rent['rent_money'];
        $result['result'][$k]['rent_start_time'] = date("Y-m-d",$rent['rent_start_time']);
        if($rent['rent_end_time']){
            $result['result'][$k]['rent_end_time'] = date("Y-m-d",$rent['rent_end_time']);
        }else{
            $result['result'][$k]['rent_end_time'] = "/";
        }
        
        $result['result'][$k]['is_pay_ok'] = $rent['is_pay_ok'];
        $result['result'][$k]['relet'] = $rent['relet'];
    }
...
{	
    ...
    $smarty->assign('ur_here',      '租赁设置');
    $smarty->assign('action_link',  array('href' => 'supplier_rent.php?act=list&status=1', 'text' => '返回租金管理'));
	
     $sql  = 'SELECT * FROM ' .$ecs->table('supplier'). ' AS al '.
            'JOIN ' .$ecs->table('supplier_rent'). ' AS u ON u.supplier_id = al.supplier_id '. $where;
    
	$supplier = $db->getRow($sql);
        
        if(!$supplier){
             $sql  = 'SELECT * FROM ' .$ecs->table('supplier'). ' AS al '.
            'left JOIN ' .$ecs->table('supplier_rent'). ' AS u ON u.supplier_id = al.supplier_id '. $where;    
            $supplier = $db->getRow($sql);
            $supplier['supplier_id'] =  $suppid;
            $supplier['rent_start_time'] = date("Y-m-d");
            $supplier['rent_end_time'] = date("Y-m-d");
            $smarty->assign('method_action',"INSERT");
            
        }else{            
            $supplier['rent_start_time'] = date("Y-m-d",$supplier['rent_start_time']);
            $supplier['rent_end_time'] = date("Y-m-d",$supplier['rent_end_time']);     
            $smarty->assign('method_action',"UPDATE");
        }
        $supplier['is_pay_ok'] = $supplier['is_pay_ok'];
        $smarty->assign('sinfo',$supplier);
        
	/* 列表页面 */
    assign_query_info();
    $smarty->display('supplier_rent_edit.htm');
}
elseif($_REQUEST['act'] == 'saveinfo')
{
	admin_priv('supplier_manage');
	$suppid = intval($_REQUEST['suppid']);
        $save['rent_start_time'] = strtotime(htmlspecialchars($_REQUEST['rent_start_time']));
        $save['rent_end_time'] = strtotime(htmlspecialchars($_REQUEST['rent_end_time']));
        $save['rent_money'] = $_REQUEST['rent_money'];
        $save['relet'] = intval($_REQUEST['relet']);
        $save['supplier_id'] = $suppid;
        
        if(intval($_REQUEST['relet'])){            
            $sql = 'INSERT INTO '.$ecs->table('supplier_rent_history').'(rent_id,supplier_id,is_pay_ok,pay_type,pay_time,remark,admin_user,rent_money,rent_img,status,rent_start_time,rent_end_time,relet) select rent_id,supplier_id,is_pay_ok,pay_type,pay_time,remark,admin_user,rent_money,rent_img,status,rent_start_time,rent_end_time,relet from '.$ecs->table('supplier_rent').'where supplier_id='.$suppid;
            $db->query($sql);      
            $save['is_pay_ok'] = 0;
        }
	
        if($_REQUEST['method_action']=='INSERT'){        
            if ($db->autoExecute($ecs->table('supplier_rent'), $save, 'INSERT')){
		$link[] = array('text' => '返回租金管理', 'href' => 'supplier_rent.php?act=list&status=1');
            sys_msg('操作成功!', 0, $link);
            }
        }else{
            if ($db->autoExecute($ecs->table('supplier_rent'), $save, 'UPDATE', "supplier_id='$suppid'")){
		$link[] = array('text' => '返回租金管理', 'href' => 'supplier_rent.php?act=list&status=1');
            sys_msg('操作成功!', 0, $link);
            }
        }
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    if($_REQUEST['flag']=='out'){
        make_json_result($smarty->fetch('supplier_rent_out.htm'), '',
        array('filter' => $result['filter'], 'page_count' => $result['page_count']));
    }elseif($_REQUEST['flag']=='nopay'){
        make_json_result($smarty->fetch('supplier_rent_nopay.htm'), '',
        array('filter' => $result['filter'], 'page_count' => $result['page_count']));
    }elseif($_REQUEST['flag']=='history'){
        make_json_result($smarty->fetch('supplier_rent_history.htm'), '',
        array('filter' => $result['filter'], 'page_count' => $result['page_count']));
    }else{
...
// 	 $status = intval($_REQUEST['status']);
	...
	$supplier['type_name'] = $type_name;
     ...
     $smarty->display('supplier_rent_info.htm');
...
elseif ($_REQUEST['act']== 'out')
{
    ...
    $smarty->assign('ur_here', "逾期未结"); // 当前导航
   ...
   $smarty->assign('supplier_list',    $result['result']);
    /* 显示模板 */
    assign_query_info();
    $smarty->display('supplier_rent_out.htm');
}

/*------------------------------------------------------ */
//-- 租金待结商户
/*------------------------------------------------------ */
elseif ($_REQUEST['act']== 'nopay')
{
     /* 检查权限 */
     admin_priv('supplier_manage');
...
elseif ($_REQUEST['act']== 'history')
{
    ...
    $smarty->assign('ur_here', "历史结算"); // 当前导航
   ...
   $smarty->assign('supplier_list',    $result['result']);
    /* 显示模板 */
    assign_query_info();
    $smarty->display('supplier_rent_history.htm');
}
...
function history_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('supplier_rent_history'). $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('supplier_rent_history'). $where.
